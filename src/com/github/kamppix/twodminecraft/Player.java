...
import com.github.kamppix.twodminecraft.commands.ChatInputBox;
import com.github.kamppix.twodminecraft.graphics.Font;
import com.github.kamppix.twodminecraft.graphics.Sprite;
import com.github.kamppix.twodminecraft.graphics.Textures;
import com.github.kamppix.twodminecraft.inventory.ContainerScreen;
import com.github.kamppix.twodminecraft.inventory.ContainerType;
import com.github.kamppix.twodminecraft.inventory.Inventory;
import com.github.kamppix.twodminecraft.inventory.PlayerInventory;
import com.github.kamppix.twodminecraft.inventory.SmallCraftingGrid;
import com.github.kamppix.twodminecraft.items.Item;
import com.github.kamppix.twodminecraft.items.ItemStack;
import com.github.kamppix.twodminecraft.items.ToolItem;
...
import com.github.kamppix.twodminecraft.util.Timer;
import com.github.kamppix.twodminecraft.util.Vector2d;
...
public class Player {
	...
	private void collideAndMove() {
		...
		if (onGround && sneaking && nextY == y) {
			if (vX > 0) collisionTile = world.collisionCheck(nextX, y - 0.5d, false);
			if (vX < 0) collisionTile = world.collisionCheck(nextX + width, y - 0.5d, false);
			if (collisionTile == null) {
				return;
			}
		}
		
		collisionTile = world.collisionCheck(nextX, y, width, height, false);
		if (collisionTile != null) {
		...
		collisionTile = world.collisionCheck(x, nextY, width, height, false);
		if (collisionTile != null) {
	...
	public void update() {
		...
		/// HEIGHT ADJUSTMENT ///
		if (!sneaking && world.collisionCheck(x, y, width, 1.8d, false) == null) {
			height = 1.8d;
		} else {
			height = 1.5d;
		}
	...
	public void input(MouseHandler mouse, KeyHandler key) {
		...
		if (guiScreen != null) {
			...
			if (key.inventory.clicked) {
				guiScreen = new ContainerScreen(this, new SmallCraftingGrid(), ContainerType.INVENTORY);
			}
			if (key.escape.clicked) {
				heal(1.0f);
			}
			if (key.layerSwap.down) {
				layerSwap = true;
			} else {
				layerSwap = false;
			}
			...
			pointPos = rayCast(eyePos.x, eyePos.y, mousePos.x, mousePos.y, 4.5F, layerSwap);
	...
	public void render(Graphics2D g, Font font, MouseHandler mouse) {
		...
		/// AIMING TILE ///
		if (world.collisionCheck(Math.floor(pointPos.x), Math.floor(pointPos.y), layerSwap) != null) {
		...
		if (getInv().getItems().get(-2) != null) {
			g.drawImage(Textures.OFFHAND_SLOT, GamePanel.width / 2 - 240, GamePanel.height - 44, 44, 44, null);
			getInv().getItems().get(-2).render(g, font, new Vector2f(GamePanel.width / 2 - 234, GamePanel.height - 38));
		}
	...
	private Vector2d rayCast(double x1, double y1, double x2, double y2, double maxLength, boolean wallTile) {
		...
		for (int i = 0; i < 300; i++) {
			...
			if (Math.sqrt(checkDist.x * checkDist.x + checkDist.y * checkDist.y) > maxLength || world.collisionCheck(checkPos.x, checkPos.y, wallTile) != null) {
				return checkPos;
			}
		}
		return checkPos;
	...
	private void place(int faceX, int faceY, int x, int y) {
		if (world.getTile(faceX, faceY, layerSwap) == null) return;
		if (getSelectedItem() == null) return;
		if (world.placeTile(x, y, Tiles.getById(getSelectedItem().getType().getId()), layerSwap)) {
			getInv().decreaseItemCount(selectedItemSlot);
	...
	private void mine(int x, int y) {
		if (world.getTile(x, y, layerSwap) == null) return;
		...
		if (world.getTile(x, y, layerSwap).getHarvestLevel() == 0) {
			miningTime = world.getTile(x, y, layerSwap).getHardness() * 1.5f;
		} else {
			miningTime = world.getTile(x, y, layerSwap).getHardness() * 5.0f;
		}
			
		if (item != null && tool != null) {
			if (tool.getMaterial().getHarvestLevel() >= world.getTile(x, y, layerSwap).getHarvestLevel() - 1) {
				miningTime = world.getTile(x, y, layerSwap).getHardness() * 1.5f;
			}
			
			if (tool.getToolType().getEffectiveOn().contains(world.getTile(x, y, layerSwap).getMaterial())) {
		...
		if (miningProgress >= 1.0f) {
			...
			if (item != null && tool != null && tool.getMaterial() != null) {
				world.destroyTile(x, y, tool.getMaterial().getHarvestLevel(), layerSwap);
			} else {
				world.destroyTile(x, y, 0, layerSwap);
			}	
		}
	}
