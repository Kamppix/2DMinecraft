package com.github.kamppix.twodminecwaft.tiles;

public class Material {
	
	public static final Material PLANTS = new Material();
	public static final Material WOOD = new Material();
	public static final Material ICE = new Material();
	public static final Material METAL = new Material();
	public static final Material PISTON = new Material();
	public static final Material CONDUIT = new Material();
	public static final Material SHULKER_BOX = new Material();
	public static final Material RAIL = new Material();
	public static final Material ROCK = new Material();
	public static final Material LEAVES = new Material();
	public static final Material WEB = new Material();
	public static final Material WOOL = new Material();
	public static final Material GROUND = new Material();
	public static final Material SNOW = new Material();
	public static final Material BAMBOO = new Material();
	public static final Material LIVING = new Material();
	public static final Material EQUAL = new Material();
	public static final Material INSTANT = new Material();
	public static final Material UNBREAKABLE = new Material();

	public Material() {
		
	}
}
