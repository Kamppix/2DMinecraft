...
public class Command {
	...
	private void giveCommand(String args) {
		...
		if (Items.getById(id) != null) {
			player.getInv().addItem(new ItemStack(Items.getById(id), count));
			System.out.println("Gave " + count + " [" + Items.getById(id).getName() + "] to " + player.getName());
		} else {
	...
	private void clearCommand(String args) {
		...
		if (id != null && !id.isEmpty() && Items.getById(id) != null) {
			for (int i = 0; i < 36; i++) {
				if (player.getInv().getItems().get(i) != null && player.getInv().getItems().get(i).getType() == Items.getById(id)) {
					if (maxCount == -1) {
						count += player.getInv().getItems().get(i).getCount();
						player.getInv().setItemCount(i, 0);
					} else {
	...
	private void setTileCommand(String args) {
		...
		if (Tiles.getById(id) != null) {
			world.setTile(x, y, Tiles.getById(id));
